...
%syntax_error {
    ...
    start ::= expr(A).   { echo  A ; }
	expr(A)::= LPAREN expr(B) RPAREN.{echo A; A= "(".B.")"; }
    ...
    expr(A) ::= expr(B) AND_OPERATOR  expr(C).   { echo A; A = B ." and ". C; }
    expr(A) ::= expr(B) OR_OPERATOR  expr(C).   { echo A; A = B ." or ". C; }
    expr(A) ::= expr(B) SOME_OPERATOR  expr(C).   { echo A; A = B ." some ". C; }
    expr(A) ::= NOT_OPERATOR  expr(B).   { echo A; A=" not " . B;}
    expr(A) ::= expr(B) ONLY_OPERATOR  expr(C).   { echo A; A = B ." only ". C; }
