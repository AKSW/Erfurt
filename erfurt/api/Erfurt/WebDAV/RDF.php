...
class HTTP_WebDAV_Server_RDF extends HTTP_WebDAV_Server
{
	 ...
	 * @var unknown_type
	 */
	private $Erfurt_selectedModel;

	/* end of my code (c.riess) */
	...
	function HTTP_WebDAV_Server_RDF($optionen)
	{
		...
		$this -> erfurt = new Erfurt_App_Default($optionen['erfurt_config']);
		...
		if($this->origin=="DB") {
			$this -> DB = $optionen['DB'];
			$this -> Erfurt_Store = $this -> erfurt -> getStore();
			//TODO CLEANUP
			...
			$this -> Erfurt_Models = $this -> Erfurt_Store -> listModels();
		...
		function PROPFIND(&$options, &$files)
		{
			...
			if (!empty($options["depth"]))  { // TODO check for is_dir() first?
				...
				$arPath = explode('/',urldecode($options['path']));
				
				if (sizeof($arPath) > 2) {
					$this -> Erfurt_selectedModel = $this -> Erfurt_Store -> getModel(str_replace('|','/',$arPath[1]));
					//var_dump($this -> Erfurt_selectedModel -> modelURI);
				}
				
				if  ($path == "/") {
					foreach($this -> Erfurt_Models as $model) {
						$this->Ausgabe_formal('Ausgabe',str_replace('/','|',$model->modelURI),$files,'Resource');
					}
				} else if (sizeof($arPath) == 3) {
					$this->Ausgabe_formal("Ausgabe","classes",$files,"Resource");
					$this->Ausgabe_formal("Ausgabe","resources",$files,"Resource");
				} else if ($arPath[2] == 'classes') {
					include("include/Classes.php");
				} else if($arPath[2] == 'resources') {
...
//			mysql_free_result($res);
		...
		function GET(&$options)
		{
			...
			$this -> Erfurt_selectedModel = $this -> Erfurt_Store -> getModel(str_replace('|','/',$this->components[1]));
			$uriResource = $this->components[sizeof($this->components)];
		...
		function PUT(&$options) {
			...
			$file = stream_get_contents($options['stream']);
			$options["path"] = $this->_slashify($options["path"]);
			$arPath = explode('/',urldecode($options['path']));
			
			$filetype = substr($arPath[sizeof($arPath)- 2],strrpos($arPath[sizeof($arPath)- 2],'.')+1);
			...
			$tempModel->load($file, $filetype, false);
			$this -> Erfurt_selectedModel -> addModel($tempModel);
...
//			return "200 OK";
		...
		$result=$this->Erfurt_selectedModel->sparqlQuery($Super_Query);
