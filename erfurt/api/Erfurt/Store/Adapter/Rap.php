...
//TODO: ERRORCODE
        try {
            // check the schema version of the database (if available) and update the schema if necessary
            $dbSchemaVersion = $this->_getDbSchemaVersion();
            
            if ($dbSchemaVersion < self::DB_SCHEMA_VERSION) {
                $this->_updateDbSchema($dbSchemaVersion, DB_SCHEMA_VERSION);
                throw new Erfurt_Exception('Database schema migrated.', 2704);
            }
            ...
            if (!$this->isSetup()) {
                throw new Erfurt_Exception('Database Setup: Checking for tables ... no tables found.', 2702);
            }
            // else check whether db schema is pre 0.85 (no ef_meta table available)
            else if ($e->getCode() == 2703) {
                $this->_updateDbSchema(-1, self::DB_SCHEMA_VERSION);
                throw new Erfurt_Exception('Database schema migrated.', 2704);
...
******************************************************************************/
    ...
    private function _getDbSchemaVersion() {
        
        $dbResult = $this->dbConn->GetOne('SELECT schema_version FROM ef_meta');
        
        if ($dbResult) {
            return floatval($dbResult);
        } else {
            throw new Erfurt_Exception('Schema version is not available.', 2703);
        }
    }
    
    /**
     * This private method converts a db schema from a old version to a newer version
     * 
     * @param float $from The old version of the schema (if available), else -1.
     * @param float $to The new version of the schema.
     */
    private function _updateDbSchema($from, $to) {

        if (($from < 0.85) && ($to == 0.85)) {
            $this->_updateDbSchemaTo085();
        }
    }
    
    /**
     * This method updates a pre 0.85 db schema to the 0.85 db schema.
     * 
     * Additions: ef_meta table with schema_version field and value 0.85
     * Modifications:
     *                  - cache: resource -> ef_resource
     *                  - log_actions: date -> ef_date, user -> ef_user
     *                  - popularity: date -> ef_date
     *                  - ratings: user -> ef_user, resource -> ef_resource
     */
    private function _updateDbSchemaTo085() {
        
        switch (strtolower($this->dbDriver)) {
            case 'mysql':
            case 'mysqli':
                $this->_updateDbSchemaTo085Mysql();
                break;
        }
    }
    
    private function _updateDbSchemaTo085Mysql() {

        $this->dbConn->startTrans();
        
        // drop indices for modified tables
        $this->dbConn->execute('ALTER TABLE cache DROP INDEX c_func_args_model_res_idx');
        $this->dbConn->execute('ALTER TABLE log_actions DROP INDEX loga_user_idx');
        $this->dbConn->execute('ALTER TABLE ratings DROP UNIQUE INDEX r_model_user_res_idx');
        
        // alter the tables
        $this->dbConn->execute('ALTER TABLE cache CHANGE COLUMN resource ef_resource VARCHAR(255) COLLATE ascii_bin');
        $this->dbConn->execute('ALTER TABLE log_actions CHANGE COLUMN date ef_date DATETIME, CHANGE COLUMN user ef_user VARCHAR(255) COLLATE ascii_bin');
        $this->dbConn->execute('ALTER TABLE popularity CHANGE COLUMN date ef_date TIMESTAMP');
        $this->dbConn->execute('ALTER TABLE ratings CHANGE COLUMN user ef_user VARCHAR(255) COLLATE ascii_bin, CHANGE COLUMN resource ef_resource VARCHAR(255) COLLATE ascii_bin');
        
        // create the dropped indices again
        ...
        // finally add the new ef_meta table
        $this->dbConn->execute('CREATE TABLE ef_meta (schema_version CHAR(6) COLLATE ascii_bin NOT NULL)');
        $this->dbConn->execute("INSERT INTO ef_meta (schema_version) VALUES ('0.85')");
