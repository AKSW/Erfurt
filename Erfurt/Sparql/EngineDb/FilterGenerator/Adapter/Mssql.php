...
class Erfurt_Sparql_EngineDb_FilterGenerator_Adapter_Mssql extends Erfurt_Sparql_EngineDb_FilterGenerator
{
    /**
    *   SQL Generator
    ...
    */
    public function createFilterSql($tree, $bOptional, $nUnionCount)
    {
        ...
        // SQLSRVCHANGE - Hardgecodet f?r MSSQL - wo kann man den Operator ?ndern?
...
#return ' AND ' . $this->createTreeSql($tree, null);
    ...
    */
    protected function createValue($tree, $bDumbParent)
    {
        ...
        if (Erfurt_Sparql_Variable::isVariable($strValue)) {
            ...
            if ($bDumbParent) {
                ...
                //SQLSRVCHANGE - Query umgeschrieben - m?sste im MYSQL laufen!
                return self::mkVal(
                    '(CASE'
                    . ' WHEN ' . $this->getDatatypeCol($tree) . '=\'' . self::$typeXsdBoolean . '\''
                        . ' THEN CASE WHEN LOWER(' . $this->getValueCol($tree) . ')=\'true\'THEN \'True\' ELSE \'False\' END'
                    . ' WHEN ' . $this->getDatatypeCol($tree) . '!=\'' . self::$typeXsdBoolean . '\''
                    . ' THEN ' . $this->getValueCol($tree)
                    . ' END)',
                    self::$typeVariable
                );
            } else {
          ...
          (!isset($tree['datatype']) || $tree['datatype'] != self::$typeXsdBoolean)
            ...
            switch ($tree['datatype']) {
                case self::$typeXsdBoolean:
                    ...
                    return self::mkVal(
						//SQLSRV CHANGE - TRUE to True - m?sste in MYSQL Laufen
    ...
    protected function createEquation($tree)
    {
        ...
        if ($this->isObject($tree['operand1'])) {
            if ($this->bOptional) {
                $strIsNull    = '(';
                $strIsNullEnd = ' OR ' . $val1 . '=\'\')'; //SQLSRVCHANGE " to'
            }
    ...
    */
    protected function createTypeEquation($operator, $val1, $val2, $tree1, $tree2)
    {
        ...
        //SQLSRVCHANGE zus?tzliche abfrage - im listview kam im debugmodus eine fehlermeldung
        if(isset($val1->type)&&isset($val2->type)) {
            if ($val1->type != self::$typeVariable && $val2->type != self::$typeVariable) {
            //both are not variables -> type needs to be the same
            //SQLSRVCHANGE "AND False" not existing in SQLSRV
...
//                }
//                else {
                    return $val1->type == $val2->type ? '' : 'AND 1 = 0';
//                }
    ...
    */
    protected function createSingleTypeCheck($val, $tree)
    {
        if ($val->type == self::$typeXsdString) {
            ...
            return ' AND ('
                . $this->getDatatypeCol($tree) . '=\'' . $val->type . '\''//SQLSRVCHANGE " to'
                . ' OR '
                . $this->getDatatypeCol($tree) . ' ' . $this->sg->arTableColumnNames['datatype']['empty']
                . ')';
        }
        return ' AND ' . $this->getDatatypeCol($tree) . '=\'' . $val->type . '\'';//SQLSRVCHANGE " to'
    }//protected function createSingleTypeCheck($val, $tree)
    ...
    */
    protected function createFunction_bound($tree)
    {
        ...
        return self::mkVal('True', self::$typeXsdBoolean);//SQLSRVCHANGE TRUE to True
    }//protected function createFunction_bound($tree)
    ...
    */
    protected function createFunction_isliteral($tree)
    {
        ...
        return self::mkVal(
            $this->isPlainString($tree) ? 'True' : 'False',//SQLSRVCHANGE TRUE to True
            self::$typeXsdBoolean
        );
    }//protected function createFunction_isliteral($tree)
