    ...
    */
    ...
    {    
        if ($this->queryString !== null) {
            ...
            if ($this->base !== null) {
                $queryString .= 'BASE <' . $this->base . '>' . PHP_EOL;
            }
            
            //set the PREFIX definitions
            foreach ($this->prefixes as $prefix=>$iri) {
                $queryString .= 'PREFIX ' . $prefix . ': <' . $iri . '>' . PHP_EOL;
            }
            
            switch ($this->resultForm) {
                case 'select':
                case 'select distinct':
                case 'select reduced':
                    if ($this->resultForm === 'select') {
                        $queryString .= 'SELECT ';
                    } else if ($this->resultForm === 'select distinct') {
                        $queryString .= 'SELECT DISTINCT ';
                    } else if ($this->resultForm === 'select reduced') {
                        $queryString .= 'SELECT REDUCED ';
                    }

// TODO disallow mixing vars and *
                    foreach ($this->resultVars as $var) {
                        $queryString .= (string)$var . ' ';
                    }
...
// TODO is the order of the FROM and FROM NAMED clauses important???
            foreach ($this->fromPart as $fromIri) {
                $queryString .= 'FROM <' . $fromIri . '>' . PHP_EOL; 
            }
                    
            foreach ($this->fromNamedPart as $fromNamedIri) {
                $queryString .= 'FROM NAMED <'. $fromNamedIri . '>' . PHP_EOL;
            }
