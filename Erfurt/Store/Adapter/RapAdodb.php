<?php
// TODO fit to new store layout

require_once 'Erfurt/Store/Abstract.php';
require_once 'Erfurt/Store/SparqlInterface.php';
require_once 'Erfurt/Store/SqlInterface.php';
require_once 'Erfurt/Store/CountableInterface.php';
...
class Erfurt_Store_Adapter_RapAdodb extends Erfurt_Store_Abstract 
	...
	public function __construct() {
		...
		$dbDriver = $config->database->params->type; // e.g. mysql, mysqli
		$host = $config->database->params->host; // e.g. localhost
		$dbName = $config->database->params->dbname; // the name of the database
		$user = $config->database->params->username;
		$password = $config->database->params->password;
		$SysOntURI = $config->SysOntModelURI; // the uri for the system ontology
		$tablePrefix = $config->database->tableprefix;
		
		$this->_dbConn = ADONewConnection($dbDriver);
		
		$r = $this->_dbConn->Connect($host, $user, $password, $dbName);
		if ($r !== true) {
		...
		#	$this->_dbConn->fnExecute = 'pwlRewriteSQL';
		#}
...
// TODO
	 ...
	 */
	public function executeSparql($model = null, $query, $class = null, $renderer = null, $useImports = true, 
			$useAcl = true) {
...
// TODO code
		...
		require_once 'Erfurt/Sparql/EngineDb/Adapter/RapAdodb.php';
		$engine = new Erfurt_Sparql_EngineDb_Adapter_RapAdodb($this->getDbConn(), $modelIDs);
		
		if ($renderer === null)	 {
			require_once 'Erfurt/Sparql/EngineDb/ResultRenderer/RapAdodb/Plain.php';
			$renderer = new Erfurt_Sparql_EngineDb_ResultRenderer_RapAdodb_Plain();
		}
		...
		$result = $engine->queryModel($query, $renderer);
	...
	public function getModel($modelURI, $importedURIs=array(), $useACL = true) {
		...
		if (is_numeric($modelURI)) {
			$modelURI = $this->_dbConn->getOne('SELECT modelURI FROM models WHERE modelID=' . $modelURI);
		}
...
// TODO throw an exception?!
		...
		if ($this->_dbConn->getOne($sql) > 0) {
			// type is owl
			require_once 'Erfurt/Owl/Model.php';
			$m = new Erfurt_Owl_Model('default', $modelURI);
		} else {
			// type is rdfs
			require_once 'Erfurt/Rdfs/Model.php';
			$m = new Erfurt_Rdfs_Model('default', $modelURI);
		}
		
		// check for edit possibility
		if ($useACL and $this->checkAc()) {
			$m->setEditable($this->ac->isModelAllowed('edit', $modelURI));
		} else {
			$m->setEditable(true);
		}
...
// TODO error code
	...
	public function isDatabaseCreated($dbType="MySQL", $dbName) {
		switch (strtolower($dbType)) {
			case "mysql":
				$db =& $this->_dbConn->execute('USE "' . $dbName . '"');
				if (!$db) return false;
				else return true;
		}
	}
	...
	public function loadModel($modelURI,$file=NULL,$loadImports=false,$stream=false,$filetype=NULL) {
		...
		if(in_array(strtolower(Zend_Registry::get('config')->database->params->type), array('mysql','mysqli')))
			$this->_dbConn->Execute('ALTER TABLE statements DISABLE KEYS');
	 	$model->load($file,$filetype,$stream);
		if(in_array(strtolower(Zend_Registry::get('config')->database->params->type), array('mysql','mysqli')))
			$this->_dbConn->execute('ALTER TABLE statements ENABLE KEYS');
...
******************************************************************************/
	...
	function _createTables_MySQL() {
		...
		// table: models
		$this->_dbConn->execute("CREATE TABLE models (
		...
		$this->_dbConn->execute('CREATE UNIQUE INDEX m_modelURI_idx ON models (modelURI)');

		// table: statements
		$this->_dbConn->execute("CREATE TABLE statements (
		...
		// table: namespaces
		$this->_dbConn->execute("CREATE TABLE namespaces (
		...
		$this->_dbConn->execute('CREATE INDEX s_modelID_idx ON statements (modelID)');
		$this->_dbConn->execute('CREATE INDEX s_subject_idx ON statements (subject)');
		$this->_dbConn->execute('CREATE INDEX s_predicate_idx ON statements (predicate)');
		$this->_dbConn->execute('CREATE INDEX s_object_idx ON statements (object(50))');
		$this->_dbConn->execute('CREATE INDEX s_sub_pred_idx ON statements (subject,predicate)');
		$this->_dbConn->execute('CREATE INDEX s_pred_obj_idx ON statements (predicate,object(50))');
		$this->_dbConn->execute('CREATE INDEX s_sub_obj_idx ON statements (subject,object(50))');
		$this->_dbConn->execute('CREATE INDEX s_subjectis_idx ON statements (subject_is)');
		$this->_dbConn->execute('CREATE INDEX s_objectis_idx ON statements (object_is)');
		$this->_dbConn->execute('CREATE INDEX s_llang_idx ON statements (l_language)');
		$this->_dbConn->execute('CREATE INDEX s_ldtype_idx ON statements (l_datatype)');
		
		$this->_dbConn->execute('CREATE FULLTEXT INDEX s_object_ft_idx ON statements (object)');
		
		// index: namespaces table
		// are multiple indices ok???
		//$this->_dbConn->execute('CREATE INDEX n_modelID_idx ON namespaces (modelID)');

		// table: popularity
		$this->_dbConn->execute("CREATE TABLE popularity (
									id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
		...
		// table: ratings
		$this->_dbConn->execute("CREATE TABLE ratings (
									id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
									modelID INT UNSIGNED NOT NULL,
		...
		$this->_dbConn->execute('CREATE UNIQUE INDEX r_model_user_res_idx ON ratings (modelID,ef_user,ef_resource)');
		
		// table: cache
		$this->_dbConn->execute("CREATE TABLE cache (
									id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
		...
		$this->_dbConn->execute('CREATE UNIQUE INDEX c_func_args_model_res_idx ON cache (function, args, model, ef_resource)');
		
		// table: log_statements
		$this->_dbConn->execute("CREATE TABLE log_statements (
									id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
									subject VARCHAR(255) COLLATE ascii_bin NOT NULL,
									predicate VARCHAR(255) COLLATE ascii_bin NOT NULL,
									object LONGTEXT COLLATE utf8_bin,
		...
		// index: action_id
		$this->_dbConn->execute('CREATE INDEX logs_action_idx ON log_statements (actionID)');
		
		// table: log_actions
		$this->_dbConn->execute("CREATE TABLE log_actions (
									id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
		...
		$this->_dbConn->execute('CREATE INDEX loga_modelid_idx ON log_actions (model_id)');
		$this->_dbConn->execute('CREATE INDEX loga_parentid_idx ON log_actions (parent_id)');
		$this->_dbConn->execute('CREATE INDEX loga_user_idx ON log_actions (ef_user)');
		
		// table: log_action_descr
		$this->_dbConn->execute("CREATE TABLE log_action_descr (
									id INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
									description VARCHAR(255) COLLATE ascii_general_ci NOT NULL) ENGINE MyISAM");
									
		// index: description
		$this->_dbConn->execute('CREATE INDEX logad_descr_idx ON log_action_descr (description(250))');
							
		if (!$this->_dbConn->completeTrans()) {
	...
	private function _createTables_Oracle() {
		...
		$this->_dbConn->execute("ALTER TABLE models ADD
		...
		$this->_dbConn->execute("ALTER TABLE statements ADD
		...
		$this->_dbConn->execute("ALTER TABLE namespaces ADD
		...
		$this->_dbConn->execute("ALTER TABLE popularity ADD
		...
		$this->_dbConn->execute("ALTER TABLE ratings ADD
		...
		$this->_dbConn->execute("ALTER TABLE cache ADD
									CONSTRAINT pk_cache
									PRIMARY KEY(id)
									USING INDEX COMPUTE STATISTICS;");
		...
		$this->_dbConn->execute("ALTER TABLE log_statements ADD
		...
		$this->_dbConn->execute("ALTER TABLE log_actions ADD
		...
		$this->_dbConn->execute("ALTER TABLE log_action_descr ADD
