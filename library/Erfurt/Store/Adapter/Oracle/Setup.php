...
class Erfurt_Store_Adapter_Oracle_Setup
{
    ...
    protected function createInsertProcedure()
    {
        ...
        $packageBodyLines = array(
            ...
            '        triples triple_list := triple_list();',
            '    BEGIN',
            '        triples.extend(graphs.count);',
            '        FOR i IN 1 .. graphs.count LOOP',
            '            triples(i) := SDO_RDF_TRIPLE_S(',
            '                graphs(i),',
            '                subjects(i),',
            '                predicates(i),',
            '                objects(i)',
            '            );',
            '        END LOOP;',
            '        FORALL i IN triples.first .. triples.last',
            '            INSERT INTO erfurt_semantic_data (triple) VALUES (triples(i));',
            '    END;',
            'END ERFURT;'
        );
