...
class Erfurt_Cache_Backend_QueryCache_Database extends Erfurt_Cache_Backend_QueryCache_Backend
{
     ...
     *  Furthermore there are some indexes which are created. 
        ...
        if (!in_array('ef_cache_query_objectkey', $existingTableNames)) {
            $columnSpec = array(
                ...
                'objectkey' =>  'VARCHAR(255)  '.
                                $vocabulary['col_ascii_bin'].
                                ' NOT NULL, PRIMARY KEY (qid, objectkey)'
           );
     ...
     *  Furthermore triplePatterns from given QueryString and ModelIris (from clause) 
     *  will also be saved . If array of modelIris is empty the QueryId will be assigned to 
     *  a NULL-Entry . Last But not Least the duration of the originally processed query will be saved.
     *  @access     public
     *  @param      string    $queryId        Its a hash of the QueryString
     *  @param      string    $queryString    SPARQL Query as String
     *  @param      array     $graphUris      extracted from the From and FromNamed Clause
     *  @param      array     $triplePatterns extracted from the Where Clause of the SPARQL Query
     *  @param      string    $queryResult    the QueryResult
     *  @param      float     $duration       duration of originally executed Query in seconds, microseconds
    ...
    public function save($queryId, $queryString, $modelIris, $triplePatterns, $queryResult, $duration = 0)
    {
        ...
        if (false === $this->exists($queryId)) {
            ...
            $queryResult = $this->_encodeResult($queryResult);
            $this->_query(
                'UPDATE ef_cache_query_result SET   result = \''.$queryResult.'\', 
                                                    duration = '.$duration.' 
                 WHERE qid = \''.$queryId.'\''
            );
        }
     ...
     *  @access     public
     *  @param      string          $queryId Its a hash of the QueryString
     *  @return     string/boolean  $result  If a result was found it returns the result or false
     */
    ...
    public function incrementHitCounter($queryId)
    {
        switch ($this->store->getBackendName()) {
            ...
            case 'MySql' :
                ...
                $this->_query(
                    'UPDATE ef_cache_query_result SET hit_count = '.($count[0]['hit_count']+1).' 
    ...
    public function incrementInvalidationCounter($queryId)
    {
        switch ($this->store->getBackendName()) {
            ...
            case 'MySql':
                ...
                $this->_query(
                    'UPDATE ef_cache_query_result SET inv_count = '.($count[0]['inv_count']+1).'
                     WHERE qid=\''.$queryId.'\' AND result IS NOT NULL'
                );
                break;
            case 'Virtuoso':
    ...
    public function invalidate ($modelIri, $statements = array())
    {
        ...
        $query = '
            ...
            ) second ON first.qid1 = second.qid2 
            JOIN 
        ...
        $query = '
            UPDATE ef_cache_query_result SET result = NULL 
            WHERE 
            (
                qid IN 
                ( 
                ...
                (
                    ...
                    WHERE ( ef_cache_query_model.modelIri = \''.$modelIri.'\' OR 
    ...
    public function saveTransactions($queryId, $transactions)
    {
        ...
        foreach ($keys as $key) {
            ...
            if (!(isset($ret[0]['qid']))) {
                $query = '
                    INSERT INTO ef_cache_query_objectkey (qid, objectkey) 
                    VALUES (\''.$queryId.'\', \''.$key.'\')' ;
                $this->_query($query);
            }
        }
    ...
    private function _saveTriplePatterns ($queryId, $triplePatterns)
    {
        ...
        foreach ($triplePatterns as $triplePattern) {
            ...
            $res = $this->_query(
                'SELECT * FROM ef_cache_query_rt 
                 WHERE qid = \''.$queryId.'\' AND tid = \''.$tripleId.'\''
            );
            if (sizeof($res) == 0) {
    ...
    private function _saveModelIris ($queryId, $modelIris)
    {
        ...
        foreach ($modelIris as $modelIri) {
            ...
            $query = '
                SELECT mid FROM ef_cache_query_model WHERE modelIri ' .
                (($modelIri == 'NULL') ? 'IS ' : '= '). $modelIri;
            $result = $this->_query($query);
